...
public class JoystickToDrive extends CommandBase {
  ...
  // Referencing the Drivetrain (we'll need a Drivetrain to drive... right?) and
  // two Double Suppliers to refer to the continuously changing Joystick inputs.
  ...
  // Execute method
  // This method is CONTINUALLY called. Meaning it's used for adjusting to
  // constantly changing inputs
  // Ex. Using the output of a control loop, or in this use case: matching
  // joystick inputs.
  @Override
  public void execute() {
    if (!HasRunOnce){
      HasRunOnce = true;
      landing_gear.setDriveType(Auton ? NeutralMode.Brake : NeutralMode.Coast);
    }
    ...
    // Driver would like for reverse to be linear & scaled down for fine control of
    // input
    ...
    // causing us to flip. a 55% top 'reverse' speed allows us to be on the tipping
    // point
  ...
  // End method
  // This method is called ONLY ONCE, but ONLY AFTER THE COMMAND IS INTERRUPTED OR
  // ENDS. Meaning it's used for one-time actions after the command is over.
  ...
  // It should be false for most times as we typically "interrupt" our Commands to
  // end them, but use it to set custom parameters you need to happen.
  // Ex. Forcing a autonomous command to end when you drive a certain distance, or
  // forcing a motor to stop if it gets too hot.
  @Override
